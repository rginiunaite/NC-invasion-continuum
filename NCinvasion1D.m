function [t,y]=NCinvasion1D(varargin)
% function [t,y]=NCinvasion1D(varargin)
%
...
path('../rowmapmex-v0.93-20141028/rowmap',path); 
...
switch integrator
 ...
 case 'rowmap'
  ...
  options.AbsTol = tol;
  options.RelTol = tol;
  options.RHSnonautonomous = 1; % the MOL-ODE system is depending on t. 
...
c = y(3:3:end);
% solution plot
plot(grid.x2,u,'b', grid.x2,v,'b--', grid.x2,c,'g');
title(['t=' num2str(t)])
axis([0 0.2 -0.1 1.1])
drawnow
pause(0.5)
