function nonlocal = setupIntegralRule2D(dir, R, h, N1, N2, Omega, ...
					ruleId, NR, varargin)
% nonlocal = setupIntegralRule2D(dir, R, h, N1, N2, Omega, ...
% 				 ruleId, NR, varargin)
%
...
% dir, R, h, N1, N2, ruleId ... copies of the respective input
...
nonlocal.Wx1  = Wx1;
nonlocal.Wx2  = Wx2;
nonlocal.Vx1FFT = setupMvFFT(nonlocal.Wx1, nonlocal.N1, nonlocal.N2, ...
					   nonlocal.lm, nonlocal.km); 
nonlocal.Vx2FFT = setupMvFFT(nonlocal.Wx2, nonlocal.N1, nonlocal.N2, ...
					   nonlocal.lm, nonlocal.km); 
if (comparison)
  nonlocal.Wx1b  = Wx1b;
  nonlocal.Wx2b  = Wx2b;
...
viFFT = fft(vi);                          % Compute FFT of the columns of vi


% Compute the permutation P in vector permv
permv = NaN*ones(1,N1*N2);
for block = 1:N1
  for blockrow = 1:N2
    permv(block+(blockrow-1)*N1) = blockrow+(block-1)*N2; 
  end
end

% Compute the vectors FFT(delta_j), j=1:N2, as the columns of deltajFFT
delta = viFFT(:);
deltajFFT = fft(reshape(delta(permv), N1, N2));

% construct return data structure
ret.N1        = N1;
ret.N2        = N2;
ret.viFFT     = viFFT;
ret.deltajFFT = deltajFFT;
ret.permv     = permv;
