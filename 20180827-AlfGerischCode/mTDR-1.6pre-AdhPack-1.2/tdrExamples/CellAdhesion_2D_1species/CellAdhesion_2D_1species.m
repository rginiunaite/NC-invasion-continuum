function [t,y]=CellAdhesion_2D_1species(varargin)
...
%    params.gridCells       = 100;
%    params.BCs             = 'pppp'; % or 'zzzz'
...
if (strcmp(flag,'init'))
  ...
  save('Data/saved.mat', 'saved');
  disp(['Saved to Data/saved.mat at time t = ' num2str(t(1))]);
  if (doPlot)
...
if (~mod(length(saved.t), saveEvery))
  save('Data/saved.mat', 'saved');
  disp(['Saved to Data/saved.mat at time t = ' num2str(t(end))]);
end
...
% solution plot
surf(grid.x1,grid.x2,reshape(y, length(grid.x2),length(grid.x1)) );
shading interp
colorbar
