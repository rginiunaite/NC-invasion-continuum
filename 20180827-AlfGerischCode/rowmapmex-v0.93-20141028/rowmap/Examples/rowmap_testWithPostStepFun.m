function rowmap_testWithPostStepFun
% function rowmap_testWithPostStepFun
%
% This example script solves a simple (diagonal) 3-dimensional
% constant coefficient linear ODE system with the ROWMAP time
% integration scheme for one tolerance value. An example of a
% post-step function is given, which serves as a basis for
% user-supplied, more advanced post-step functions; here we store
% the numerical solution for certain time points in a global
% variable. 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% File    : rowmap_testWithPostStepFun.m
...
if (strcmp(flag,'init'))
  ...
  % is completed, then the solution can be accessed via the global
  % sol variable. 
  sol.out.tspan = [t(1) 0.2:0.2:0.7 t(end)]; % define output time points
  sol.out.count = 0;
  sol.out.t =[];
  sol.out.y =[];

  % check if first output time is initial time
  ...
  % the following fields are only for small systems and testing
  % purposes:
  sol.count = 1;
  sol.t     = t(1);
  sol.ypre  = y'; % store computed solution at end of time step
  sol.ypost = y'; % store modified solution at end of time step
...
% continues (set status to 2) to indicate that
status = 2;
ynew = y+1e-4*(rand(size(y))-0.5);

% you can return now -- then the rhs at ynew will be computed
% internally and fnew is ignored -- or you set status to 3 and
% provide the fnew value, e.g. by updating only those components
% which need updating or simply alling your RHS function, as is
% done here, for ynew 
status = 3;
fnew = rhs(t,ynew);

% store results (for large systems, the solution after each time
% step should not be stored for memory reasons). 
sol.count              = sol.count+1;
sol.t(sol.count)       = t;
sol.ypre(sol.count,:)  = y';
sol.ypost(sol.count,:) = ynew';
